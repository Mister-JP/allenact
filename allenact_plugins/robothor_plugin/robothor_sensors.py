...
class GPSCompassSensorRoboThor2(Sensor[RoboThorEnvironment, PointNavTask]):
    def __init__(self, uuid: str = "target_coordinates_ind2", **kwargs: Any):
        observation_space = self._get_observation_space()
    ...
    def _compute_pointgoal(
        ...
        # print(source_rotation)
        # print("computing_pointgoal from goal and source!")
        direction_vector = source_position
        direction_vector_agent = GPSCompassSensorRoboThor.quaternion_rotate_vector(
            source_rotation.inverse(), direction_vector
        )
        ...
        # return np.array([rho, phi], dtype=np.float32)
        return np.array([goal_position[0], goal_position[2], rho, phi], dtype=np.float32)
...
class GPSCompassSensorRoboThor4(Sensor[RoboThorEnvironment, PointNavTask]):
    def __init__(self, uuid: str = "target_coordinates_ind4", **kwargs: Any):
        observation_space = self._get_observation_space()
